...
class Invoice extends MX_Controller {
    ...
    public function kpntwks_invoice_list(){
    ...
    public function kpntwks_invoice_details($invoice_id = null){
    ...
    public function kpntwks_invoice_pad_print($invoice_id){
           $invoice_detail = $this->invoice_model->retrieve_invoice_html_data($invoice_id);
         $taxfield = $this->db->select('*')
                ->from('tax_settings')
                ->where('is_show',1)
        ...
        'customer_email'   => $invoice_detail[0]['customer_email'],
        'final_date'       => $invoice_detail[0]['final_date'],
        'print_setting'    => $this->invoice_model->kpntwks_print_settingdata(),
        'invoice_details'  => $invoice_detail[0]['invoice_details'],
    ...
    public function kpntwks_invoice_pos_print($invoice_id = null){
        $invoice_detail = $this->invoice_model->retrieve_invoice_html_data($invoice_id);
        $taxfield = $this->db->select('*')
                ->from('tax_settings')
                ->where('is_show',1)
    ...
    public function kpntwks_pos_print_direct(){
    ...
    public function kpntwks_download_invoice($invoice_id = null){
        $invoice_detail = $this->invoice_model->retrieve_invoice_html_data($invoice_id);
        $taxfield = $this->db->select('*')
                ->from('tax_settings')
                ->where('is_show',1)
    ...
    public function kpntwks_manual_sales_insert(){
        ...
        if ($this->form_validation->run() === true) {
            ...
            if(!empty($invoice_id)){
                ...
                if($normal == 1){
                    $printdata       = $this->invoice_model->kpntwks_invoice_pos_print_direct($invoice_id);
    ...
    public function kpntwks_edit_invoice($invoice_id = null){
    ...
    public function kpntwks_update_invoice(){
     $this->form_validation->set_rules('customer_id', display('customer_name') ,'required|max_length[15]');
  ...
  public function kpntwks_gui_pos(){
            $taxfield = $this->db->select('tax_name,default_value')
                ->from('tax_settings')
                ->get()
                ->result_array();
 ...
 public function kpntwks_customer_autocomplete(){
     ...
     /*product autocomple search*/
        public function kpntwks_autocomplete_product(){
        $product_name   = $this->input->post('product_name',TRUE);
     ...
     /*after selecting product retrieve product info*/
        ...
        public function kpntwks_batchwise_productprice() {
   ...
   $createdate = date('Y-m-d H:i:s');
    ...
    $customer_coa = [
         ...
         echo json_encode($data);
            ...
            public function kpntwks_invoice_details_directprint($invoice_id = null){
         $invoice_detail     = $this->invoice_model->retrieve_invoice_html_data($invoice_id);
        $taxfield = $this->db->select('*')
                ->from('tax_settings')
                ->where('is_show',1)
   ...
   /*invoice no generator*/
    ...
    function kpntwks_terms_list() {
      ...
      public function kpntwks_terms_form($id = null)
    {
    ...
    public function kpntwks_terms($id = null) {
