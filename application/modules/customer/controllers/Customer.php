...
class Customer extends MX_Controller {
    ...
    function index_group() {
        $data['title']             = display('customer_group');
        $data['module']            = "customer";
        $data['page']              = "customer_group"; 
        $data['all_customer']      = $this->customer_model->allcustomergroup(); 
        echo modules::run('template/layout', $data);
    }


    public function kpntwks_CheckCustomerList(){
        $postData = $this->input->post();
        $data     = $this->customer_model->getCustomerList($postData);
        echo json_encode($data);
    }

    public function kpntwks_CheckCustomerGroupList(){
        $postData = $this->input->post();
        $data     = $this->customer_model->getCustomerGroupList($postData);
        echo json_encode($data);
    }

     public function kpntwks_credit_customer() {
        $data['title']             = display('credit_customer');
        $data['module']            = "customer";
        $data['page']              = "credit_customer"; 
        $data["customer_dropdown"] = $this->customer_model->kpntwks_credit_customer_dropdown();
        $data['all_customer']      = $this->customer_model->kpntwks_all_credit_customer(); 
        echo modules::run('template/layout', $data);
    }

     public function kpntwks_CheckCreditCustomerList(){
    ...
    public function kpntwks_paid_customer() {
        $data['title']             = display('paid_customer');
        $data['module']            = "customer";
        $data['page']              = "paid_customer"; 
        $data["customer_dropdown"] = $this->customer_model->kpntwks_paid_customer_dropdown();
        $data['all_customer']      = $this->customer_model->kpntwks_all_paid_customer(); 
        echo modules::run('template/layout', $data);
        
    }
    
     public function kpntwks_CheckPaidCustomerList(){
        // GET data
        $postData = $this->input->post();
        $data = $this->customer_model->kpntwks_getPaidCustomerList($postData);
        echo json_encode($data);
    } 


  public function kpntwks_form($id = null)
    {
        ...
        $this->form_validation->set_rules('group_category','required');
        ...
        $data['customer'] = (object)$postData = [
            ...
            'group_id'         => $this->input->post('group_category',true),
        ...
        if ($this->form_validation->run() === true) {
            ...
            $data['group']    = $this->customer_model->allcustomergroup();  
            $data['module']   = "customer";  
            $data['page']     = "form";  
            echo Modules::run('template/layout', $data); 
        }else{
    ...
    public function kpntwks_form_group($id = null)
    {
        $data['title'] = display('add_customer_group');
        if(empty($_POST)){
            $data['module']            = "customer";
            $data['page']              = "customer_group_add"; 
            if($id != null){
                $data['customer'] = $this->customer_model->singledatagroup($id); 
            }
            echo modules::run('template/layout', $data);
        }else{
            if (empty($_POST['id'])) {
                ...
                if ($this->customer_model->create_group($data)) {
                    #set success message
                        $info['msg']    = display('save_successfully');
                        $info['status'] = 1;
                } else {
                ...
                $this->customer_model->update_group($data, $id);
            }
    ...
    public function kpntwks_delete($id) {
    ...
    public function kpntwks_group_delete($id) {
        if ($this->customer_model->delete_group($id)) {
            echo display('delete_successfully');
        } else {
            display('please_try_again');
        }
    ...
    public function kpntwks_customer_ledger() {
    ...
    public function kpntwks_customer_ledgerData() {
    ...
    public function kpntwks_customer_advance() {
