...
class Customer_model extends CI_Model {
  ...
  //credit customer dropdown
    public function kpntwks_credit_customer_dropdown()
  {
  ...
  // paid customer dropdown
   public function kpntwks_paid_customer_dropdown()
  {
    $data =  $this->db->select("a.*,b.HeadCode,((select ifnull(sum(Debit),0) from acc_transaction where COAID= `b`.`HeadCode`)-(select ifnull(sum(Credit),0) from acc_transaction where COAID= `b`.`HeadCode`)) as balance")
      ->from('customer_information a')
      ->join('acc_coa b','a.customer_id = b.customer_id','left')
	...
	public function customer_list($offset=null, $limit=null)
    {
      ...
      public function getCustomerList($postData=null){
         ...
         $searchQuery = "";
         if($searchValue != ''){
            $searchQuery = " (a.customer_name like '%".$searchValue."%' or a.customer_mobile like '%".$searchValue."%' or c.group_name like '%".$searchValue."%' or a.customer_email like '%".$searchValue."%'or a.phone like '%".$searchValue."%' or a.customer_address like '%".$searchValue."%' or a.country like '%".$searchValue."%' or a.state like '%".$searchValue."%' or a.zip like '%".$searchValue."%' or a.city like '%".$searchValue."%') ";
         }
         ...
         $this->db->join('customer_group c','a.group_id = c.id_group');
         ...
         ## Fetch records
         $this->db->select("a.*,c.group_name,b.HeadCode,((select ifnull(sum(Debit),0) from acc_transaction where COAID= `b`.`HeadCode` AND IsAppove = 1)-(select ifnull(sum(Credit),0) from acc_transaction where COAID= `b`.`HeadCode` AND IsAppove = 1)) as balance");
         $this->db->from('customer_information a');
         $this->db->join('acc_coa b','a.customer_id = b.customer_id','left');
         $this->db->join('customer_group c','a.group_id = c.id_group');
         $this->db->group_by('a.customer_id');
         ...
         foreach($records as $record ){
          ...
          $base_url = base_url();
            ...
            $data[] = array( 
                ...
                'category_name'    =>$record->group_name,
        ...
        public function getCustomerGroupList($postData=null){
         ...
         $searchQuery = "";
         if($searchValue != ''){
            $searchQuery = " (group_name like '%".$searchValue."%') ";
         }
         ...
         foreach($records as $record ){
          $button = '';
          $base_url = base_url();
              $button .='<a href="'.$base_url.'edit_customer_group/'.$record->id_group.'" class="btn btn-info btn-xs m-b-5 custom_btn" data-toggle="tooltip" data-placement="left" title="Update"><i class="pe-7s-note" aria-hidden="true"></i></a><a onclick="customergroupdelete('.$record->id_group.')" href="javascript:void(0)"  class="btn btn-danger btn-xs m-b-5 custom_btn" data-toggle="tooltip" data-placement="right" title="Delete "><i class="pe-7s-trash" aria-hidden="true"></i></a>';   
    ...
    //paid customer list
     public function kpntwks_getPaidCustomerList($postData=null){
    ...
    public function singledatagroup($id = null)
    {
        return $this->db->select('*')
            ->from('customer_group')
            ->where('id_group', $id)
  ...
  public function kpntwks_all_credit_customer(){
   ...
   return $data =  $this->db->select("a.*,b.HeadCode,((select ifnull(sum(Debit),0) from acc_transaction where COAID= `b`.`HeadCode`)-(select ifnull(sum(Credit),0) from acc_transaction where COAID= `b`.`HeadCode`)) as balance")
    ...
    public function kpntwks_all_paid_customer(){
